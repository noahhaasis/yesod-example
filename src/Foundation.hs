...
instance Yesod App where
    ...
    defaultLayout widget = do
        ...
        let menuItems =
                ...
                , NavbarLeft $ MenuItem
                    { menuItemLabel = "Critical"
                    , menuItemRoute = CriticalR
                    , menuItemAccessCallback = isJust muser
                    }
                , NavbarRight $ MenuItem
                    { menuItemLabel = "Login"
    ...
    isAuthorized CriticalR isWrite = isAuthenticatedAndManager isWrite
...
instance YesodBreadcrumbs App where
    ...
    breadcrumb CriticalR = return ("Critical", Just HomeR)
...
-- | Acess function to determine whether a user can view "critical site" and do modifications.
--   Every user can access the critical site but only managers can perform the critical operations
--   on the users they manage.
isAuthenticatedAndManager :: Bool -> Handler AuthResult
isAuthenticatedAndManager False = return Authorized -- Everyone can access the critical site
isAuthenticatedAndManager True = do
  muid <- maybeAuthId
  case muid of
     Nothing -> return $ Unauthorized "You must login to perform this task"
     Just uid -> do
       musername <- lookupPostParam "username"
       case musername of
         Just name -> uid `isManagerOf` name >>= \case
            True -> return Authorized
            False -> return $ Unauthorized "You're not the manager"
